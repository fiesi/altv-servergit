...
var view = null;
var tempPedId = 0;
function updatePedHair(data) {
    if (tempPedId === 0) {
        return;
    }
    natives.setPedComponentVariation(tempPedId, 2, data.hair, 0, 0);
    natives.setPedHairTint(tempPedId, Number(data.hairColorPrimary), Number(data.hairColorSecondary));
}
async function updatePed(data, clothing) {
    if (tempPedId === 0) {
        await createPed(data);
        return;
    }
    if (data !== null) {
        natives.setPedHeadBlendData(tempPedId, Number(data.father), Number(data.mother), 0, Number(data.fatherSkin), Number(data.motherSkin), 0, Number(data.faceMix), Number(data.skinMix), 0, false);
    }
    if (clothing !== null) {
        updatePedHair(clothing);
    }
}
async function createPed(data) {
    if (tempPedId !== 0) {
        natives.deletePed(tempPedId);
    }
    const modelHash = alt.hash(data.sex === 0 ? "mp_m_freemode_01" : "mp_f_freemode_01");
...
alt.onServer(CharacterEvent.CreationCreate, () => {
    ...
    view.on('character:creation:create:ped', async (data) => await createPed(data));
    view.on('character:creation:update:ped', async (data, clothing) => await updatePed(data, clothing));
    view.focus();
...
alt.onServer(CharacterEvent.CreationError, (msg) => view.emit(CharacterEvent.CreationError, msg));
