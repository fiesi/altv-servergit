<template>
  <p>{{ test }}</p>
  <div class="absolute bottom-[2.917vw] left-2/4 translate-x-[-50%]">
    <div class="flex gap-4">
      <div v-for="(char, elId) in characters" :key="char.id" @click="selectCharacter(char.id)" class="min-w-[13.1vw]">
        <div class="h-[5vw] overflow-hidden">
          <img v-if="elId === 0" class="w-[5vw]" src="@/assets/images/select/men.png" alt="">
          <img v-else class="w-[5vw]" src="@/assets/images/select/woman.png" alt="">
        </div>
        <div class="h-[5.7vw] bg-[#008736] border-[0.15vw] border-[#008736] border-opacity-30 bg-opacity-30 hover:bg-opacity-50 rounded-md cursor-pointer p-4">
          <div class="flex items-center">
            <div class="text-base">
              <p class="font-semibold">{{ char.firstname }} {{ char.lastname }} [{{ char.id }}]</p>
              <p class="text-gray-300">${{ char.money }}</p>
              <p class="text-gray-300">${{ char.bank }}</p>
            </div>
          </div>
        </div>
      </div>
      <div v-if="characters.length < 2" @click="createCharacter()" class="h-full mt-auto">
        <div class="h-[5.7vw] min-w-[13.1vw] bg-white border-[0.15vw] border-opacity-30 bg-opacity-30 hover:bg-opacity-50 rounded-md cursor-pointer p-4 bottom-0">
          <div class="h-full flex flex-col justify-center items-center">
            <p class="text-base font-semibold">Erstellen</p>
          </div>
        </div>
      </div>
      <div v-else class="h-full mt-auto">
        <div class="h-[5.7vw] min-w-[13.1vw] bg-white border-[0.15vw] border-opacity-30 bg-opacity-30 hover:bg-opacity-50 rounded-md cursor-pointer p-4 bottom-0">
          <div class="h-full flex flex-col justify-center items-center">
            <svg class="h-12" fill="#fff" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 574.922 574.922" xml:space="preserve">
              <g>
                <g>
            ...
            <p class="text-base font-semibold">Gesperrt</p>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    alt.on('character:selection:create', (chars) => {
      chars.forEach((el) => {
  ...
  methods: {
    selectCharacter(id) {
      alt.emit('character:select', id);
    },
    createCharacter() {
      alt.emit('character:create');
    }
  }
}
</script>